...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // 1. Check if user has scrolled away from top
      setIsScrolled(currentScrollY > 20);

      // 2. Hide/Show Logic - Don't hide if mobile menu is open
      if (!isMobileMenuOpen) {
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between h-full w-full">
          ...
          {/* DESKTOP LINKS */}
          <div className="hidden lg:flex items-center gap-8 text-sm font-bold tracking-widest uppercase text-gray-900">
          ...
          <button 
            ...
            className="lg:hidden text-2xl text-black p-2 z-[110] hover:scale-110 transition-transform"
      ...
      {/* --- MOBILE MENU OVERLAY --- */}
      <div className={`fixed inset-0 bg-white z-[90] lg:hidden transition-transform duration-500 ease-in-out pt-24 overflow-y-auto
...
const NavLink = ({ to, label, isActive }) => (
  ...
  >
    {label}
    <span className={`absolute left-0 bottom-0 h-[2px] bg-black transition-all duration-300 ${isActive ? 'w-full' : 'w-0 hover:w-full'}`}></span>
  </Link>
);
...
const DropdownItem = ({ to, label }) => (
  <Link 
    to={to} 
    className="px-6 py-3 text-[11px] font-bold text-gray-500 hover:text-black hover:bg-gray-50 uppercase tracking-widest transition-colors"
  >
    {label}
  </Link>
);
...
const MobileLink = ({ to, label, onClick, isSmall }) => (
  <Link 
    ...
    className={`transition-colors duration-300 ${
      isSmall 
      ? 'text-xs font-bold text-gray-500  hover:text-black tracking-widest' 
      : 'text-xs font-bold text-gray-500 hover:text-black tracking-widest'
    }`}
  >
    {label}
